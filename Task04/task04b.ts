...
export const task4b = () => 
{
    ...
    //const fileName = "Task04/test_day4.txt"
    ...
    let completedBoards : BingoBoard[] = []
    ...
    for(let index = 0; index < randomNumbers.length; index++){
        ...
        completedBoard.forEach(board => completedBoards.push(board))
        boards = boards.filter(board => !board.hasBoardWon())

        if (completedBoards.length == boardCount) {
            winningBoard = completedBoards[completedBoards.length - 1]
            break
        }
    }
